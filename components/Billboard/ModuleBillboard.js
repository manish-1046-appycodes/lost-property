...
const ModuleBillboard = ({ settings }) => {
  ...
  const contentAlignmentVerticalMob =
    settings.contentPosition.contentAlignmentVerticalMob === "bottom"
      ? "pt-[255px]"
      : "pb-[255px]";
  const contentAlignmentVertical =
    settings.contentPosition.contentAlignmentVertical === "bottom"
      ? "md:justify-end md:pb-0 md:pt-36"
      : "md:justify-start md:pt-0 md:pb-36";

  const imageMobPad =
    settings.contentPosition.contentAlignmentVerticalMob == "bottom"
      ? "top-0 bottom-[150px] md:bottom-0"
      : "bottom-0 top-[150px] md:top-0";
  ...
  if (settings.billboardType === "img_full") {
    ...
    if (settings.contentPosition.contentAlignment == "right") {
      ButtonRoundAlignmentMD +=
        " md:left-1/3 md:translate-x-[10%] md:bottom-0 md:mb-[120px]";
    } else if (settings.contentPosition.contentAlignment == "left") {
      ButtonRoundAlignmentMD +=
        " md:left-2/3 md:-translate-x-full md:bottom-0 md:mb-[120px]";
    } else {
      ButtonRoundAlignmentMD +=
        " md:left-1/3 md:translate-x-[10%] md:bottom-0 md:mb-[120px]";
    }
  ...
  const ButtonRoundWrap = ButtonRoundUrl &&
    ButtonRoundClass &&
    ButtonRoundTitle && (
  ...
  return (
    <section
      className={`relative ${
        settings?.settings?.marginBottom ? "mb-20 lg:mb-28" : ""
      } ${settings?.settings?.marginTop ? "mt-20 lg:mt-28" : ""}`}
    >
          ...
          >
            <div
              className={`w-9/12 w-[280px] md:w-6/12 max-w-[598px] p-4 lg:px-14 lg:py-9 text-center  flex flex-col justify-between items-center space-y-4 md:relative ${
                settings.contentTextColour
              } ${settings.contentBackgroundColour} ${
                settings.contentPosition.contentAlignment === "left"
                  ? "md:-left-1/4"
                  : ""
              } ${
                settings.contentPosition.contentAlignment === "right"
                  ? "md:left-1/4"
                  : ""
              }`}
            >
              ...
              <div
                className={` ${
                  !settings?.contentImage?.sourceUrl ? "py-6 lg:py-20" : ""
                } space-y-4 pb-4`}
              >
                <div
                  className={`py-0 wysiwyg ${
                    settings?.copy_expanded ? "max-w-[470px]" : "max-w-[470px]"
                  }`}
              ...
              {settings?.copy_expanded && (
                <div
                  className={`lg:hidden bottom-0 right-0 ml-auto uppercase flex items-center justify-end cursor-pointer space-x-3 ${
              ...
              {settings?.ctaLink?.title && settings?.ctaLink?.url && (
                <div className="ml-auto">
                  <ButtonSecondary
                    title={settings.ctaLink.title}
                    url={settings?.ctaLink.url}
                  />
                </div>
              )}

              {((ButtonRoundWrap && settings.billboardType == "caption_img") ||
                settings.billboardType == "caption_full") &&
                ButtonRoundWrap}
            </div>
      ...
      {settings.billboardType === "img_full" && (
        <div
          ref={ref2}
          className={` overflow-hidden absolute w-full top-0  left-0 ${imageMobPad}`}
        >
        ...
        <>
          {settings.backgroundCaption && (
            <div className={captionCaptionClass}>
              <div className="container">
                <div
                  className="heading-brand-medium"
                  dangerouslySetInnerHTML={{
                    __html: settings.backgroundCaption,
                  }}
                />
              </div>
            </div>
