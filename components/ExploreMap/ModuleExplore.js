      ...
      let feature = {
        ...
        geometry: {
          ...
          coordinates: [
            ...
            location?.mapCoordinates?.lattitude,
          ],
        },
        properties: {
    ...
    if (mapCategorySelected == "poi-All") {
      ...
      setGeojsonFiltered({
        ...
        features: geojson.features.filter(
          (property) => property.properties.category === filter
        ),
      });
    }
      ...
      {geojson && (
        <div className="relative">
          <div className="flex flex-col lg:block">
            <div
              className={` ${
                showMap ? "!visible !opacity-100" : ""
              } order-2 opacity-0 invisible map-wrap top-0 z-[9] transition-[visibility,opacity] duration-500 fixed`}
            >
              <ExploreMap
                mapCategories={mapCategories}
                category={mapCategorySelected}
                geojson={geojson}
            ...
            <section
              className={` ${
                !showMap ? "!visible !opacity-100" : "opacity-0 invisible"
              } order-3 container relative min-h-screen map-offset_mob lg:map-offset_lg transition-[visibility,opacity] duration-500 flex justify-center flex-col`}
            >
              <div className="h-[120px] lg:h-[0px]"></div>
              <div className="absolute w-full h-full top-0 left-0 container">
                <div className="sticky  top-0 min-h-screen z-20 flex items-center justify-center flex-col">
                  <h1 className="heading-brand-medium text-center">
                    Get <em>lost</em> in <br />
                    the <em>City</em>
                  </h1>
                </div>
              </div>
                ...
                {geojsonFiltered &&
                  geojsonFiltered?.features.map((location, i) => {
                    ...
                    location?.properties?.title
                      ? (caption = location.properties.title)
                      : null;
                    ...
                    return (
                      <div
                        key={i}
                        className={`explore-item mt-32 lg:mt-0 w-9/12 lg:w-6/12 transform ${classOuter}`}
                      >
                          ...
                          <div className={`relative  ${classInner}`}>
                            <div className="relative pt-[100%] cursor-wrap">
                              ...
                              <div
                                ...
                                data-latlng={location.geometry.coordinates}
                                onClick={mapSelect}
                              />
                            </div>

                            {caption && (
                              <div className="absolute uppercase text-xs leading-13 lg:text-16px lg:leading-13 top-full left-0 mt-4">
                                {caption}
                              </div>
                            )}
                          </div>
            ...
            <div
              className={`z-50 top-0 lg:top-auto filter-bar bottom-0 h-[56px] lg:h-[73px] relative z-[9] order-1 lg:order-2 ${
                filterOpen ? "filterOpen" : ""
              }`}
            >
              <div
                onClick={toggleFilter}
                className={`cursor-pointer w-full bottom-0 left-0  opacity-50 backdrop-blur-xl min-h-mob_min_height lg:min-h-lg_min_height bg-cream-1 absolute ${
              ...
              <div className="absolute top-0 lg:top-auto lg:bottom-0 left-0 border-t border-b border-black bg-cream-1 w-full">
                <div
                  className="h-[56px] lg:h-[73px] flex justify-center items-center  cursor-pointer"
                  onClick={toggleFilter}
                >
                  <div
                    onClick={toggleMap}
                    className="absolute h-full w-[62px] bg-black flex left-0 bottom-0 h-[56px] lg:h-[73px]"
                  >
                    <div className={`m-auto ${showMap ? "hidden" : "block"}`}>
                      <svg
                        ...
                        width="25.83"
                        height="38.237"
                        viewBox="0 0 25.83 38.237"
                      >
                        <path
                          id="Path_132"
                          data-name="Path 132"
                          d="M1268.925,104.02a12.415,12.415,0,0,0-12.415,12.415c0,6.857,12.415,24.455,12.415,24.455s12.415-17.6,12.415-24.455A12.415,12.415,0,0,0,1268.925,104.02Zm0,16.486a3.465,3.465,0,1,1,3.465-3.465A3.465,3.465,0,0,1,1268.925,120.506Z"
                          transform="translate(-1256.01 -103.52)"
                          fill="none"
                          stroke="#faf7f3"
                          strokeMiterlimit="10"
                          strokeWidth="1"
                  ...
                  <div className="flex items-center space-x-6">
                    <button
                      ...
                      className="z-[9] transform duration-500 text-grey-1 relative w-[26px] h-[26px]"
                      aria-hidden="true"
                    >
                ...
                <div
                  ref={filterWrap}
                  className="filter-wrap h-0 overflow-hidden transition-all duration-500 ease-in-out"
                >
                  <ul className="flex  flex-col  text-[30px] lg:text-[50px] uppercase text-center">
                    <li className="group border-t border-grey-1 flex justify-center py-2">
                      <a
                        onClick={setCategory}
                        data-layer={`poi-All`}
                        href="#"
                        className="group-hover:font-display inline-block"
                      >
                        all
                      </a>
                    </li>
                    ...
                    {mapCategories.map((mapCategory, i) => (
                      <li
                        key={i}
                        className="group border-t border-grey-1 flex justify-center py-2"
                      >
                        <a
                          onClick={setCategory}
                          data-layer={`poi-${mapCategory}`}
                          href="#"
                          className="group-hover:font-display inline-block"
                        >
                          {mapCategory}
