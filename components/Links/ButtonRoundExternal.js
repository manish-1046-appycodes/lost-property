...
const ButtonRoundExternal = ({url, bg, color, title, size, alt, target}) => {
  ...
  return (
    <a href={url} target="_blank">
        <a className={`button-round relative z-10 ${ size == 'small'  ? 'w-[62px] h-[62px]' : 'w-[86px] h-[86px]'} lg:w-[113px] lg:h-[113px] flex justify-center items-center group ${ alt == 'isAlt' ? 'text-current' : 'text-white'} cursor-pointer`}>
