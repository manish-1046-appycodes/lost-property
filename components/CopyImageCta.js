      ...
      <div className="container">
        <div className="max-w-1430px mx-auto">
          <div
            className={`
                ${
                  settings?.settings?.imagecontentAlignment == "ilcr"
          ...
          >
            <div
              className={`
                     ...
                     md:w-1/2 grow-0 ${
                       postType == "post" ? "max-w-[517px]" : "max-w-[483px]"
                     } space-y-10 relative z-10`}
            >
              <div
                className="wysiwyg flex-initial"
                dangerouslySetInnerHTML={{ __html: settings?.content?.copy }}
              />
              ...
              {settings?.title && (
                <motion.div
                  ...
                  className={`
                        ${
                          settings?.settings?.imagecontentAlignment == "ilcr"
                            ? "justify-start text-left xl:-mr-[20vw]"
                            : "justify-end text-right xl:-ml-[20vw]"
                        }
                        flex`}
                >
                  <h2
                    dangerouslySetInnerHTML={{ __html: settings.title }}
                    className="italic heading-brand-medium max-w-[1094px]"
                  />
                </motion.div>
              )}
            </div>
